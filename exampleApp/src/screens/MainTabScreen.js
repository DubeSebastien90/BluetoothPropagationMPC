  ...
  return (
    <FlatList
      ...
      contentContainerStyle={
        state.contacts.length === 0 ? ct.emptyContainer : ct.listContent
      }
      ListEmptyComponent={
        <View style={ct.emptyBox}>
          <Image source={NO_ONE} style={ct.emptyImg} resizeMode="contain" />
          <Text style={ct.emptyTitle}>No contacts yet</Text>
          <Text style={ct.emptyBody}>
            Go to Add to scan someone's QR code.
          </Text>
        </View>
      }
      renderItem={({ item }) => (
